    ...
    <div className="w-full">
      <ul className="w-full ">
        {educationDetails.map((items, index) => (
          <li key={index} className="relative flex items-baseline gap-8  pb-10">
            <div
              className={`${index === educationDetails.length - 1 ? "" : "before:absolute  before:left-[5.5px] before:h-full before:w-[1px] dark:before:bg-white before:bg-black"}`}
            >
              <svg
            ...
            <div className="flex  flex-1 gap-10 justify-between">
              <div>
                ...
                <p className="mt-3 text-sm max-w-[450px]">{items.degreeName}</p>
              </div>
              <div className="">
                <p className="w-[100px]">
                  {items.startingYear} - {items.endingYear}
                </p>
              </div>
            </div>
          </li>
